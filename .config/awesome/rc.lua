...
local delayed_spawn = function(delay, cmd, tag)
    local timer = gears.timer.start_new(delay, function()
        awful.spawn(cmd, { tag = tag })
    end)
end
...
local taglist_buttons = gears.table.join(
    ...
    awful.button({ tag_modkey }, 1, function(t)
        if client.focus then
            client.focus:move_to_tag(t)
        end
    end),
    awful.button({}, 3, awful.tag.viewtoggle),
    awful.button({ tag_modkey }, 3, function(t)
        if client.focus then
...
awful.spawn.single_instance("autorandr --change aoc_additional")
...
globalkeys = gears.table.join(
    awful.key(
        ...
        "s",
        setup_workspace_dev,
        { description = "setup a dev workspace", group = "custom" }
    ),
    ...
    end, { description = "up the volume by 10%", group = "utils" }),
    awful.key({}, "XF86AudioLowerVolume", function()
        awful.spawn("pactl set-sink-volume @DEFAULT_SINK@ -10%")
    end, { description = "down the volume by 10%", group = "utils" }),
    ...
    end, { description = "lock overlay", group = "awesome" }),
    awful.key({}, "", function()
        awful.spawn("pactl set-sink-volume @DEFAULT_SINK@ -10%")
    end, { description = "open a terminal", group = "launcher" }),
...
for i = 1, 9 do
    globalkeys = gears.table.join(
        globalkeys,
        -- View tag only.
        awful.key({ tag_modkey }, "#" .. i + 9, function()
        ...
        -- Toggle tag display.
        awful.key({ tag_modkey, "Control" }, "#" .. i + 9, function()
            local screen = awful.screen.focused()
            local tag = screen.tags[i]
            if tag then
        ...
        -- Move client to tag.
        awful.key({ tag_modkey, "Shift" }, "#" .. i + 9, function()
            if client.focus then
        ...
        awful.key({ tag_modkey, "Control", "Shift" }, "#" .. i + 9, function()
            if client.focus then
                local tag = client.focus.screen.tags[i]
                if tag then
